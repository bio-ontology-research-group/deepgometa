...
dataset = 'Blueberry'
metric = 'Function' #Function or Taxa
pheno = 'Type' #Phenotype to be tested, make sure to add all other phenotypes in features variable
k = 2

# Load the function abundance data
if metric == 'Function':
    function_abundance_path = '/path/to/function/abundance/table/' + '_function_abundance.tsv'
if metric == 'Taxa':
    function_abundance_path = '/path/to/taxa/abundance/table/' + '/otutab_relative_taxa_merged.tsv'
function_abundance_data = pd.read_csv(function_abundance_path, sep='\t', index_col=0)

# Load the metadata
metadata_path = '/path/to/metadata/table/' + '/sample_mapping.txt'
metadata = pd.read_csv(metadata_path, sep='\t')

# Transpose the function abundance data to have samples as rows
function_abundance_transposed = function_abundance_data.transpose()

# Set the index of the function abundance data to match the '16S' column in the metadata
function_abundance_transposed.index.name = '16S'
merged_data = metadata.merge(function_abundance_transposed, on='16S', how='left')
...
features = merged_data.drop(columns=['16S', 'WGS', pheno, 'Sample', 'Location'])
...
plt.title('PCA Based on Relative ' + metric + ' Abundance (' + dataset + ', ' + pheno + ')')
