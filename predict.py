...
import gzip
from deepgo_esm import DGModel, load_normal_forms
...
def main(in_file, data_root, threshold, batch_size, device):
    ...
    for ont in ['mf', 'cc', 'bp']:
        ...
        model = DGModel(2560, n_terms, n_zeros, n_rels, device).to(device)
        for mn in ent_models[ont]:
            model_file = f'{data_root}/{ont}/deepgo_esm_{mn}.th'
